...
Two empirical validations confirm the model: the structural \textit{Controfase} activation of 6--7 April 2026 ($\mu_1$) and the UAE exit from OPEC on 29 April 2026 ($\mu_2$), both predicted in timing and structural type. A third validation --- the Beijing Summit of 13--15 May 2026 ($\mu_3$), manifesting as Fragmentation$\to$Mass at the predicted date --- extends the empirical base to three independent points on the $\IC(t)$ curve, all within 3.2\% of the model fit. From these confirmed events: $\gj = 0.075\;\mathrm{IC/month}^2$ (67\% higher than the preliminary estimate, confirming that $\gj$ increases with proximity to the attractor). Retroactive determination of $t_0$ yields 5--6 February 2026 --- coinciding exactly with the original prediction.
...
\vspace{0.5em}
\noindent\textbf{Status:} v1.2 Beta --- three confirmed micro-junctions ($\mu_1$, $\mu_2$, $\mu_3$), under observation and continuous refinement. Updated versions will be released as subsequent micro-junctions are confirmed or falsified. Next checkpoint: $\mu_4$ ($\sim$31 May 2026).
\end{abstract}
...
$\gj = 0.075$ IC/month$^2$ & $S_1$ & Derived from two confirmed empirical points; consistent with third ($\mu_3$, $\Delta < 3.2\%$) \\
...
BZ model as isomorph & $S_1$ & Confirmed by two empirical validations \\
...
Teleological $\gj$ & $S_1$ & Measured 67\% increase over preliminary estimate \\
Predictions ($\mu_4, \ldots, \mu_8$) & $S_2$ & Model-derived from $S_1$ inputs \\
...
$\mu_3$ & 0.375 & 77 & \textbf{16 May} & Frag. $\to$ Mass & \textbf{Confirmed: Beijing Summit 13--15 May} \\
...
Each row carries a dual prediction: timing \emph{and} structural type. The model is falsifiable on both axes independently. $\mu_3$ was confirmed on 13--15 May 2026 as the Beijing Summit, manifesting Fragmentation$\to$Mass (structural exclusion of Europe from the decision table, public formalization of US oligarchic governance via 17 CEO delegation as state interlocutors, Xi's invocation of the Thucydides Trap as ontological parity claim). Timing: $\Delta t < 1$ day from model prediction. If $\mu_4$ arrives at the predicted date but exhibits a different structural type, or if it exhibits Mass but arrives outside the $\pm 7$ day window, the model is partially falsified and requires recalibration.
...
The model has been empirically validated by three real-time events: the structural \textit{Controfase} activation of 6--7 April 2026 ($\mu_1$), the UAE exit from OPEC on 29 April 2026 ($\mu_2$), and the Beijing Summit of 13--15 May 2026 ($\mu_3$, Fragmentation$\to$Mass). All three were predicted in timing and structural type. The recalibrated $\gj = 0.075\;\mathrm{IC/month}^2$ --- 67\% higher than the preliminary estimate --- confirms that $\gj$ increases with proximity to the attractor. All three points fall within 3.2\% of the model fit, and the curve $\IC(t)$ is now constrained by three independent observations.
...
The recalibrated model projects the next macro-junction for mid--late July 2026. Each remaining micro-junction ($\mu_4$ through $\mu_8$) carries a dual prediction --- timing and OST pathology type --- providing continuous falsifiability as the system evolves. The next checkpoint is $\mu_4$ ($\sim$31 May 2026), with expected pathology Mass (peak). Updated versions of this paper will be released as subsequent junctions are confirmed or falsified.
...
F.~Ghioni, ``The Direction Problem: On the Causal Origin of Oriented Motion in Complex Systems,'' Ordinative Sciences Foundation, May 2026, v1.0 Beta. Preprint.
